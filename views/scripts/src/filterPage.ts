class Options {
    ...
    public static slider: { [key: string]: SliderFilterOption } = {
        rooms: new SliderFilterOption("Camere", "camere").set(1, 5).openRightDomain(),
    ...
    private static changeAllOnType() {
        const typeDependent = [
            [Options.dropdown.apType, Options.slider.rooms, Options.slider.bathrooms, Options.slider.builtIn], //Apartment
    ...
    private static changePriceOnType() {
        const prices: [[number, number], [number, number]][] = [
            [[150, 2000], [50000, 400000]], //apartament inchiriere, apartament cumparare
            [[500, 4000], [100000, 400000]], //casa
            [[750, 5000], [200000, 500000]], //office
            [[2000, 10000], [300000, 600000]] //land
        ]

        const changeHandler = (__index: number, __text: string) => {
            const typeIndex = Options.dropdown.type.getOption().index;
            const transactionTypeIndex = Options.dropdown.transaction.getOption().index;
            const currentPrice = prices[typeIndex][transactionTypeIndex];
            Options.slider.price.set(...currentPrice).openRightDomain().openLeftDomain();;
        }
        ...
        Object.entries(Options.slider)
            .forEach(([key, op]) => {
                ...
                if (pos1 != null && pos2 != null)
                    op.setSliderPosition(pos1, pos2);
