...
class SearchHandler {
  public static itemsElement = document.getElementById("items") as HTMLElement; //pretty sure it exists
  ...
  public static resizeHandler() {
    ...
    else SearchHandler.itemsElement.classList.remove("chubby-items");
  }
  ...
  public static toApiParams(): string {
    const dropdownParams = Object.entries(Options.dropdown)
      ...
      .map(
        ([key, op]) =>
          `${key}=${
            SearchHandler.dropdownMapToString[key][op.getOption().index]
          }`
      );
    ...
    const sliderParams = Object.entries(Options.slider)
      ...
      .map(([key, op]) =>
        Object.entries(op.getOption())
          .map(([minmax, value]) => `${key}${minmax}=${value}`)
          .join("&")
      );
  ...
  public static getItems() {
    ...
    fetch(url)
      ...
      .then((data) => data.forEach((val) => SearchHandler.addItem(val)));
  }
}
